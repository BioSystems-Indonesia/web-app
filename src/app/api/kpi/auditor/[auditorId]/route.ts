...
export const GET = WithAuth(
  async (
    ...
    try {
      ...
      const id = params?.auditorId as string;

      if (!id) {
        return NextResponse.json(APIResponseBuilder.badRequest("Invalid auditor id"), {
          status: HttpStatus.BAD_REQUEST,
        });
      }

      const result = await useCase.getAuditsByAuditor(id);
