...
export default function LISPage() {
    ...
    const handleRevokeToggle = (device: DeviceItem) => {
        const message = device.revoked ? 'Are you sure you want to unrevoke this device?' : 'Are you sure you want to revoke this device?'
        setConfirmTitle('Confirm')
    ...
    const runConfirmAction = async () => {
        ...
        try {
            if (pendingConfirm.kind === 'delete') {
                ...
                try {
                    if (device.revoked) {
                        await axios.put(`${API_BASE}/devices/${device.machine_id}/unrevoke`, null, { headers: { 'x-api-key': API_KEY } })
                    } else {
                        await axios.put(`${API_BASE}/devices/${device.machine_id}/revoke`, null, { headers: { 'x-api-key': API_KEY } })
                    }
                    await fetchDevices()
                    success = true
                } catch (err) {
    ...
    return (
        <div className="lis-container">
            <h1><SlChemistry /> LIS Dashboard</h1>
            <div className="lis-header">
                <h1 className="lis-title"><FaKey className="title-icon" /> Licenses</h1>
            </div>
                    ...
                    <tbody>
                        {licenses.map((lic, index) => (
                            ...
                            >
                                ...
                                <td className="lis-td text-center">
                                    <button className="action-btn edit" onClick={() => openEditModal(lic)}><FaEdit /> Edit</button>
                                    <button className="action-btn delete" onClick={() => handleDelete(lic.id)} disabled={deletingId === lic.id}><RiDeleteBin6Fill /> Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <div className="lis-header">
                ...
                <h2 className="lis-title"> <FaDesktop className="title-icon" /> Devices</h2>
            </div>
                        ...
                        <table className="lis-table">
                            <thead className="lis-thead">
                                <tr>
                                    ...
                                    <th className="lis-th" style={{ textAlign: 'center' }}>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                {devices.map((d, i) => (
                                    <tr key={d.id} className={i % 2 === 0 ? 'row-even' : 'row-odd'}>
                                        ...
                                        <td className="lis-td text-center">{d.revoked ? <span className="badge-yes">Yes</span> : <span className="badge-no">No</span>}</td>
                                        <td className="lis-td text-center">
                                            <button
                    ...
                    <div className={`modal-overlay ${confirmIsClosing ? 'closing' : ''}`} onClick={() => closeConfirm()}>
                        <div className={`modal ${confirmIsClosing ? 'closing' : ''}`} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
                            ...
                            <div className="modal-footer">
                                ...
                                <button className="generate-button" onClick={runConfirmAction} disabled={confirmLoading}>{confirmLoading ? 'Processing...' : confirmPrimaryLabel}</button>
                            </div>
                        </div>
                    </div>
                )
